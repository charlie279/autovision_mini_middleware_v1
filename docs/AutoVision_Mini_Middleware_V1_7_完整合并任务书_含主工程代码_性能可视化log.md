# AutoVision Mini Middleware V1.7：完整合并任务书（主工程代码 + scripts + examples + 性能可视化 log）

> 版本定位：V1.7 是在 V1.6 `YUYV raw + fused preprocess` 分支基础上继续推进的视觉主线性能可观测版本。V1.7 不推翻 V1/V1.5/V1.6 已跑通的 CameraAdapter、SHM、Runtime、Safety、Control 主链路，而是把性能优化亮点收敛为三个可展示项：
>
> 1. 保留 V1.6 的 `--camera-output rgb|yuyv` 双路径，用 YUYV raw payload 降低 Camera→SHM 的帧数据带宽；
> 2. 在 `preprocess_node` 增加逐帧 `--perf-log` CSV，记录 input_format、input_bytes、stride、preprocess_ms、total_ms；
> 3. 新增 synthetic 与 real camera 两套 benchmark 脚本，并自动生成 HTML/SVG 可视化性能对比报告，作为面试/简历可展示的视觉链路性能优化证据。
>
> 当前 Linux 沙箱验证状态：主工程 CMake 编译通过；V1 文件输入链路回归通过，最终状态 `NORMAL`；examples 全量编译通过；`12_yuyv_fused_preprocess` 与 `13_preprocess_benchmark_compare` 通过；`visualize_perf_compare.py` 已生成 synthetic 性能对比 CSV 与 HTML。当前环境无 `/dev/video0`，真实 USB Camera RGB/YUYV pipeline 与香橙派 5 Plus 板端性能仍待实机验证。

---

## 1. 项目背景

本项目面向自动驾驶中间件岗位能力训练，目标不是做复杂感知算法，而是用一个最小可落地视觉链路覆盖 Camera/Lidar 传感器抽象、V4L2 采集、共享内存传输、前处理算子、Runtime/NPU 接口抽象、控制面通信、安全监控与性能质量把关等核心能力。

你当前已具备 USB 摄像头，并计划后续在香橙派 5 Plus（16G 内存）上移植与开发。因此，V1.7 的重点不再是重复搭建基础链路，而是沿视觉主线继续加深性能优化：从“能跑通”推进到“能对比、能量化、能可视化、能说明瓶颈”。

V1.7 对岗位能力的补强点如下：

| 岗位方向 | V1.7 覆盖方式 |
|---|---|
| Camera 传感器抽象 | `CameraAdapterV4L2` 支持 RGB888 / YUYV 双输出模式 |
| 媒体链路性能意识 | 量化 RGB888 与 YUYV raw payload 的带宽差异 |
| 前处理算子 | `image_preprocess.hpp/cpp` 抽象 RGB resize/normalize 与 YUYV fused preprocess |
| 视觉端到端质量把关 | `preprocess_node --perf-log` 逐帧记录时延与格式信息 |
| 可视化性能报告 | `visualize_perf_compare.py` 输出 HTML/SVG 对比报告 |
| 板端迁移准备 | `stride_bytes` 为后续 DMA-BUF / RGA / MIPI CSI / RKNN import 预留接口 |
...
```text
1. 保持 V1/V1.5 文件输入链路不破坏。
2. 保持 V1.5 RGB Camera pipeline 兼容：--camera-output rgb。
3. 保持 V1.6 YUYV raw + fused preprocess 能力：--camera-output yuyv。
4. FrameMeta / SensorFrame 携带 stride_bytes。
5. preprocess_node 支持 RGB888 与 YUYV 输入，并输出逐帧 perf CSV。
6. 新增 synthetic benchmark：不依赖 /dev/video0，也能验证前处理函数与可视化脚本。
7. 新增 real camera benchmark：在 VMware / 香橙派上对比 RGB 与 YUYV 两条真实 Camera 路径。
8. 新增 HTML/SVG 性能对比报告，便于截图放入工作日志、简历项目或面试材料。
```

### 2.2 V1.7 性能优化边界

```text
1. V1.7 仍不是 DMA-BUF zero-copy；FramePool 仍为 POSIX SHM + memcpy。
2. V1.7 不宣称端到端一定降低 CPU 时延；CPU YUYV fused preprocess 可能比纯 RGB resize 更耗算力。
3. V1.7 的确定性收益是 payload 带宽：640×480 RGB888 为 921600 B/frame，YUYV 为 614400 B/frame，理论减少约 33.3%。
4. 真实端到端收益必须以 VMware / 香橙派实机 camera benchmark 输出为准。
5. rknn_stub 仍是 SDK-style runtime 占位，不链接真实 RKNN。
6. safety_monitor 仍为 watchdog / CRC / frame_id / timeout 学习版，不是 ISO 26262 / ASIL 级功能安全实现。
```
...
| `preprocess_node` | consumer + producer | 读取 FrameMeta，CRC 校验，RGB/YUYV 前处理，输出 TensorMeta，并记录 perf CSV | FrameMeta Ring + FramePool | TensorPool + TensorMeta Ring + `--perf-log` |
| `npu_stub_node` | consumer | 读取 TensorMeta 和 Tensor payload，通过 Runtime backend 模拟推理 | TensorMeta Ring + TensorPool | `logs/npu_results.jsonl` + RuntimeStatus |
...
```text
1. RGB888 compact stride = width * 3。
2. YUYV compact stride = width * 2。
3. V4L2 / MIPI / ISP 输出可能存在 bytes_per_line 对齐，不能长期假设 width * bytes_per_pixel。
4. 后续接 RGA / DMA-BUF / RKNN import 时，需要同时传 width、height、format、stride。
```
...
USB Camera /dev/video0
    ...
    -> SensorFrame RGB888, 921600 B @ 640x480
    -> media_node memcpy to SHM
    -> preprocess_node RGB resize + normalize
    -> TensorPool

V1.6/V1.7 YUYV raw + fused preprocess 路径：
USB Camera /dev/video0
    -> V4L2 mmap YUYV
    -> CameraAdapterV4L2 输出 SensorFrame YUYV, 614400 B @ 640x480
    -> media_node memcpy to SHM
...
```bash
./build/preprocess_node --frames 120 --save-every 60 --perf-log logs/benchmark/camera_rgb_perf.csv
```

CSV 字段：

```text
frame_id,input_format,input_bytes,stride_bytes,preprocess_ms,total_ms,crc_errors,frame_jumps
```

可视化脚本：

```bash
python3 scripts/visualize_perf_compare.py   --input logs/benchmark/camera_rgb_perf.csv --case camera_rgb   --input logs/benchmark/camera_yuyv_fused_perf.csv --case camera_yuyv_fused   --out-csv logs/benchmark/camera_perf_compare_summary.csv   --out-html logs/benchmark/camera_perf_compare.html
```
...
sequenceDiagram
    ...
    participant TPool as Tensor SHM Pool
    participant TRing as TensorMeta Ring
    participant NPU as npu_stub_node
    participant Log as perf CSV + npu_results.jsonl

    Cam->>Adapter: V4L2 mmap YUYV frame
    Adapter->>Adapter: YUYV -> RGB888
    Adapter->>Media: SensorFrame RGB888, stride=width*3
    Media->>Pool: memcpy RGB payload
    Media->>Ring: push FrameMeta(format=RGB888,stride)
    Pre->>Ring: pop FrameMeta
    Pre->>Pool: read RGB payload
    Pre->>Pre: CRC check + RGB resize/normalize
    Pre->>Log: append perf csv row
    Pre->>TPool: write float tensor
    Pre->>TRing: push TensorMeta
    NPU->>TRing: pop TensorMeta
    NPU->>TPool: read tensor
    NPU->>Log: write JSON result
```

### 5.2 V1.7 Camera YUYV fused 路径
...
sequenceDiagram
    ...
    Cam->>Adapter: V4L2 mmap YUYV frame
    Adapter->>Adapter: copy compact YUYV payload
    Adapter->>Media: SensorFrame YUYV, stride=width*2
    Media->>Pool: memcpy YUYV payload
    Media->>Ring: push FrameMeta(format=YUYV,stride)
    Pre->>Ring: pop FrameMeta
    Pre->>Pool: read YUYV payload
    Pre->>Pre: CRC check + fused YUYV->RGB->resize->normalize
    Pre->>Log: append perf csv row
    Pre->>TPool: write float tensor
    Pre->>TRing: push TensorMeta
    NPU->>TRing: pop TensorMeta
...
### 5.3 数据面：大 payload 走 SHM，小元数据走 Ring

```mermaid
sequenceDiagram
    autonumber
    participant Producer as media_node
    participant Pool as ShmFramePool
    participant Ring as ShmRingBuffer(FrameMeta)
    participant Consumer as preprocess_node

    Producer->>Pool: memcpy frame payload to buffer[index]
    Producer->>Ring: push FrameMeta(buffer_index, crc32, format, stride)
...
### 5.5 安全面：heartbeat / CRC / frame jump / timeout
...
```text
[12_yuyv_fused_preprocess] input_format=YUYV input_bytes=614400 output_tensor_bytes=1228800 loops=50 mean_ms=... p95_ms=...
```

### 6.5 合成 RGB vs YUYV 性能对比与可视化

```bash
./scripts/benchmark_v1_7_synthetic_compare.sh 100
xdg-open logs/benchmark/synthetic_perf_compare.html
```

预期：

```text
[13_preprocess_benchmark_compare] rgb_input_bytes=921600 rgb_mean_ms=...
[13_preprocess_benchmark_compare] yuyv_input_bytes=614400 yuyv_mean_ms=...
[visualize_perf_compare] rows=... groups=2 out_csv=... out_html=...
```

本次沙箱实际 synthetic summary：

| case | format | frames | input_bytes | stride | preprocess_mean_ms | preprocess_p95_ms |
|---|---:|---:|---:|---:|---:|---:|
| rgb_baseline | RGB888 | 20 | 921600 | 1920 | 0.390 | 0.408 |
| yuyv_fused | YUYV | 20 | 614400 | 1280 | 0.920 | 0.978 |


说明：synthetic 结果只证明函数与可视化链路可运行；真实 USB 摄像头端到端收益以 camera benchmark 为准。

### 6.6 VMware / 香橙派真实 Camera RGB 路径

```bash
./scripts/run_camera_pipeline.sh /dev/video0 120 640 480 30 dummy rgb logs/benchmark/camera_rgb_perf.csv
...
### 6.7 VMware / 香橙派真实 Camera YUYV fused 路径

```bash
./scripts/run_camera_pipeline_yuyv_fused.sh /dev/video0 120 640 480 30 dummy logs/benchmark/camera_yuyv_fused_perf.csv
cat logs/final_status_camera.txt
```
...
### 6.8 真实 Camera RGB vs YUYV 可视化对比

```bash
./scripts/benchmark_v1_7_camera_compare.sh /dev/video0 120 640 480 30 dummy
xdg-open logs/benchmark/camera_perf_compare.html
```

预期产物：

```text
logs/benchmark/camera_rgb_perf.csv
logs/benchmark/camera_yuyv_fused_perf.csv
logs/benchmark/camera_perf_compare_summary.csv
logs/benchmark/camera_perf_compare.html
logs/benchmark/final_status_camera_rgb.txt
logs/benchmark/final_status_camera_yuyv.txt
```

---

## 7. examples 测试体系与每个测试案例说明

| 示例 | 测试目标 | 主要接口 | 预期输出 |
|---|---|---|---|
| `00_crc32` | CRC32 校验 | `crc32_compute()` | 打印固定 payload 的 CRC |
| `01_latency_profiler` | 延迟统计 | `LatencyProfiler::add/mean/percentile` | samples、mean、p95 |
...
| `05_shm_frame_pool` | 共享内存 payload | `ShmFramePool::create/write/read` | output="frame payload example" |
| `06_shm_ring_buffer` | 元数据 Ring | `ShmRingBuffer::create/push/pop` | frame_id=42, buffer_index=2 |
| `07_shared_status` | 共享状态 | `SharedStatus::update_node/set_safety/snapshot` | fps=15, state=NORMAL |
...
| `11_camera_adapter_v4l2` | 真实 USB Camera 单模块采集 | CameraAdapterV4L2 ioctl + mmap | 30 帧，默认 RGB888 输出，保存 PPM |
| `12_yuyv_fused_preprocess` | 合成 YUYV 融合前处理 | `resize_yuyv_to_tensor()` | mean_ms / p95_ms / tensor_mean |
| `13_preprocess_benchmark_compare` | 合成 RGB vs YUYV 性能对比 | `resize_rgb888_to_tensor()` + `resize_yuyv_to_tensor()` | CSV + summary stdout |
...
### 8.1 Linux 沙箱 / VMware 基础验收

| 编号 | 验收项 | 标准 | 当前状态 |
|---|---|---|---|
| A1 | 主工程编译 | `./scripts/build.sh` 通过 | 已验证 |
| A2 | 文件输入链路 | `final_status.txt` 为 NORMAL，120/120/120 | 已验证 |
| A3 | examples 全量编译 | `cd examples && make all` 通过 | 已验证 |
| A4 | 合成 YUYV 前处理 | example 12 输出 mean/p95 | 已验证 |
| A5 | 合成性能对比 | example 13 输出 CSV | 已验证 |
| A6 | HTML 可视化 | `visualize_perf_compare.py` 输出 HTML | 已验证 |
| A7 | RGB Camera 实机 | RGB camera pipeline NORMAL | 待 VMware / 香橙派验证 |
| A8 | YUYV Camera 实机 | YUYV camera pipeline NORMAL | 待 VMware / 香橙派验证 |
| A9 | Camera 可视化对比 | `camera_perf_compare.html` 生成 | 待 VMware / 香橙派验证 |
| A10 | 香橙派 5 Plus | ARM64 编译与 USB Camera 运行 | 待板端验证 |

### 8.2 性能验收口径

```text
1. Payload 带宽账必须能说明：RGB888 921600 B/frame，YUYV 614400 B/frame，理论减少 33.3%。
2. perf CSV 必须逐帧记录 preprocess_ms 和 total_ms。
3. HTML 报告必须包含至少 preprocess_mean_ms 与 preprocess_p95_ms 两个指标的可视化。
4. 真实 Camera benchmark 必须分别保存 RGB 与 YUYV 的 final_status、media/preprocess/npu/safety 日志。
5. 不允许把 synthetic benchmark 结果包装成真实摄像头结果。
```
...
```text
[PASS] CMake 主工程编译
[PASS] V1 文件输入链路回归：NORMAL，media/preprocess/npu 均 120 帧
[PASS] examples/Makefile 全量编译
[PASS] example 12：YUYV fused preprocess 合成测试
[PASS] example 13：RGB vs YUYV synthetic benchmark
[PASS] visualize_perf_compare.py：CSV 汇总与 HTML/SVG 输出
[PASS] benchmark_v1_7_synthetic_compare.sh：合成性能对比端到端脚本
```

### 9.2 等待验证

```text
[TODO] VMware Ubuntu /dev/video0 RGB Camera pipeline
[TODO] VMware Ubuntu /dev/video0 YUYV fused Camera pipeline
[TODO] VMware Ubuntu camera RGB vs YUYV benchmark HTML
[TODO] 香橙派 5 Plus 16G ARM64 编译
[TODO] 香橙派 5 Plus USB Camera RGB/YUYV 对比
[TODO] 后续 RGA / DMA-BUF / RKNN Runtime 接入
```
...
git pull
git checkout -b v1.7-vision-perf-visual-log
```
...
```bash
# step1: 数据结构与前处理抽象
git add include/avm_config.hpp include/sensor_frame.hpp include/frame_meta.hpp include/image_preprocess.hpp src/image_preprocess.cpp CMakeLists.txt
git commit -m "v1.7-step1: add RGB/YUYV preprocess helpers and stride metadata"

# step2: Camera 与 media/preprocess 双路径集成
git add include/camera_adapter_v4l2.hpp src/camera_adapter_v4l2.cpp src/media_node.cpp src/preprocess_node.cpp
git commit -m "v1.7-step2: support RGB and YUYV camera paths with preprocess perf log"

# step3: scripts benchmark 与可视化报告
git add scripts/run_file_pipeline.sh scripts/run_camera_pipeline.sh scripts/run_camera_pipeline_yuyv_fused.sh scripts/benchmark_v1_7_camera_compare.sh scripts/benchmark_v1_7_synthetic_compare.sh scripts/visualize_perf_compare.py scripts/run_all_vm.sh
git commit -m "v1.7-step3: add benchmark scripts and visual performance report"

# step4: examples 测试体系
git add examples/Makefile examples/cpp/12_yuyv_fused_preprocess.cpp examples/cpp/13_preprocess_benchmark_compare.cpp
git commit -m "v1.7-step4: add YUYV fused preprocess and benchmark examples"

# step5: 文档与验证记录
git add README.md docs/validation
git commit -m "v1.7-step5: document Linux validation and performance visualization workflow"

git push -u origin v1.7-vision-perf-visual-log
```
...
| 版本 | 目标 | 说明 |
|---|---|---|
| V1.7 | 视觉主线性能可观测 | RGB/YUYV 双路径、perf CSV、HTML 可视化 |
| V1.8 | 香橙派 5 Plus 移植 | ARM64 编译、USB Camera 实测、CPU/内存/温度记录 |
| V1.9 | RGA/G2D 前处理 | 将 CPU resize/format convert 迁移到 RGA/G2D，保留 CPU fallback |
| V2.0 | DMA-BUF / fd passing | 减少 SHM memcpy，打通 buffer 生命周期管理 |
| V2.1 | RKNN Runtime | 替换 rknn_stub，接入真实 RKNN SDK 或板端 NPU backend |
| V2.2 | 多摄像头 / 时间同步 | 支持多 CameraAdapter、时间戳对齐、帧丢失统计 |
| V2.3 | 更完整 Safety Monitor | timeout 分级、节点重启策略、degraded mode 明确动作 |
...
```text
这个项目不是单纯算法 Demo，而是按自动驾驶中间件链路拆出来的最小视觉系统。V1/V1.5 先跑通 File/Camera 输入、POSIX SHM、FrameMeta Ring、preprocess、Runtime stub、control 和 safety monitor。V1.6 开始进入视觉性能优化，把 CameraAdapter 的默认 RGB888 中间帧路径扩展成可选 YUYV raw 输出，并在 preprocess_node 中融合完成 YUYV 到 Tensor 的前处理。V1.7 继续增强可观测性：preprocess_node 能输出逐帧性能 CSV，脚本能自动生成 RGB 与 YUYV 的 HTML 可视化对比报告。

这个版本的核心亮点是：我不是只说“做了优化”，而是能从 payload 带宽、stride 元数据、逐帧 preprocess 延迟、P95 指标、最终 NORMAL 状态和 HTML 报告几个层面展示优化依据。后续在香橙派 5 Plus 上可以继续把 CPU fallback 替换为 RGA 或 DMA-BUF/RKNN 真实链路。
```
...
以下为 V1.7 当前建议版本的完整主工程代码、scripts 脚本和 examples 测试代码。按路径复制即可；脚本复制后执行 `chmod +x scripts/*.sh scripts/*.py`。
...
````markdown
# AutoVision Mini Middleware V1.7

Ubuntu 22.04 / VMware / Orange Pi 5 Plus 预研用的车载视觉中间件最小闭环 Demo。

V1.7 在 V1.6 `YUYV raw + fused preprocess` 基础上增加了性能可观测性：

- CameraAdapterV4L2 可选择输出 `RGB888` 或紧凑 `YUYV`；
- `FrameMeta` / `SensorFrame` 携带 `stride_bytes`，为 DMA-BUF / RGA / MIPI CSI 演进预留；
- `preprocess_node` 支持 RGB888 与 YUYV 输入，并可通过 `--perf-log` 输出逐帧 CSV；
- `examples/13_preprocess_benchmark_compare` 生成合成 RGB/YUYV 前处理性能对比；
- `scripts/visualize_perf_compare.py` 输出 CSV 汇总与 HTML/SVG 可视化报告；
- `scripts/benchmark_v1_7_camera_compare.sh` 用于在 VMware 或香橙派 5 Plus 上对比真实 USB Camera RGB 路径与 YUYV fused 路径。
...
## Synthetic preprocess benchmark
...
- POSIX SHM + memcpy 仍不是 DMA-BUF zero-copy。
- `rknn_stub` 仍是 SDK-style 占位，不链接真实 RKNN runtime。
- 当前代码包在 Linux 沙箱完成编译、文件输入链路与合成性能对比验证；真实 `/dev/video0` Camera pipeline 需在你的 VMware Ubuntu 或香橙派 5 Plus 上验证。
...
<head>
<meta charset="utf-8"/>
<title>AutoVision V1.7 Preprocess Performance Compare</title>
<style>
...
<body>
<h1>AutoVision V1.7 Preprocess Performance Compare</h1>
<p class="note">This report is generated from real benchmark CSV files. It does not invent camera results.</p>
{body}
</body>
</html>
''', encoding='utf-8')
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < loops; ++i) {
        ...
        rgb_profiler.add(ms);
        csv << "rgb_baseline,RGB888," << rgb.size() << ',' << src_width * 3U << ','
...
rgb_baseline,RGB888,20,921600,1920,0.3903151,0.4078584,0.3903151,0.4078584
yuyv_fused,YUYV,20,614400,1280,0.91966045,0.9781212499999999,0.91966045,0.9781212499999999
